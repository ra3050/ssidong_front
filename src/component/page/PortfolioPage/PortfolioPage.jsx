    ...
    if (value === "전체" || value === portfolioList[i].category) {
      ...
      concernList.push(
        <PfContent
          ...
          width={pfcontentWidth}
          height={pfcontentHeight}
          onClick={() => handle({ rows: num })}
          key={i}
        >
...
const PortfolioPage = () => {
  ...
  useEffect(() => {
    requestPortfolio((data) => {
      if (!data) {
        ...
        const newContentList = makeContentList(
          portfolioList.data,
          "전체",
          handleContent
        );
  ...
  const handleContent = (e) => {
    ...
    setDetail(
      <PortfolioDetail
        handleDetailDrop={handleDetailDrop}
        portfolio={pfFilterList}
        rows={rows}
        scrolltoTop={
  ...
  return (
    <Wrapper>
      ...
      <PortfolioBox>
        ...
        <PfContentBox width={pfboxWidth}>{contentList}</PfContentBox>
      </PortfolioBox>
    </Wrapper>
  );
};
